    ...
    return <div className="space-y-6 container mx-auto px-4 py-8">
        ...
        <Card>
            ...
            <CardContent>
                <p> {trips.length === 0 ? (
                    <>
                        Start planning your first trip by clicking <span className="font-medium">New Trip</span> above.
                    </>
                ) : (
                    ...
                    {sortedTrips.slice(0, 6).map((trip, key) => (
                        <Link key={key} href={`/trips/${trip.id}`}>
                            <Card className="h-full hover:shadow-md transition-shadow">
                                <CardHeader>
                                    <CardTitle className="line-clamp-1 leading-[1.3]">{trip.title}</CardTitle>
                                </CardHeader>

...
export default function TripDetailClient({ trip }: TripDetailClientProps) {
    ...
    return <div className="container mx-auto px-4 py-8 space-y-8"> {trip.imageUrl && <div className="w-full h-72 md:h-96 overflow-hidden rounded-xl shadow-lg relative">
        <Image src={trip.imageUrl} alt={trip.title} className="object-cover" fill priority /></div>}
        ...
        <div className="bg-white p-6 shadow rounded-lg">
            <Tabs value={activeTab} onValueChange={setActiveTab}>
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                                ...
                                <div className="flex items-start">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-500">
                                            ...
                                            <br />
                                            {(() => {
                                                const diff = Math.round((trip.endDate.getTime() - trip.startDate.getTime()) / (1000 * 60 * 60 * 24));
                        ...
                        {trip.locations.length === 0 && (
                            <div className="text-center p-4">
                                <p className="mb-2">Add locations to see them appear on the map!</p>
                                <Link href={`/trips/${trip.id}/itinerary/new`}>
                                    <Button><Plus className="mr-2 h-5 w-5" />Add Location</Button>
                                </Link>
                            </div>
                ...
                <TabsContent value="itinerary" className="space-y-6">
                    ...
                    {trip.locations.length === 0 ? (
                        <div className="text-center p-4">
                            <p className="mb-2">Add locations to see them appear on the itinerary!</p>
                            <Link href={`/trips/${trip.id}/itinerary/new`}>
                                <Button><Plus className="mr-2 h-5 w-5" />Add Location</Button>
                            </Link>
                        </div>
